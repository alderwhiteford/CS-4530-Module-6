...
export async function importGrades(gradesToImport: ImportTranscript[]): Promise<Transcript[]> {
  ...
  // for(let i = 0 ; i < gradesToImport.length ; i++) {
  //   // Retrieve the student and their grades:
  //   const { studentName, grades } = gradesToImport[i]
  ...
  //   // Fetch the transcript for the student
  ...
  // const importStudentTranscript = gradesToImport.map(async (transcript) => {
  //   const { studentName, grades } = transcript
  ...
  // await Promise.all(importStudentTranscript)
